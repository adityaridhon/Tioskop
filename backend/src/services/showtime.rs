...
fn validate_movie_id(movie_id: i64) -> Result<()> {
    if movie_id <= 0 {
        Err(ShowtimeError::Validation("Movie ID harus > 0".to_string()))
    } else {
        Ok(())
    }
}
...
fn validate_studio_id(studio_id: i64) -> Result<()> {
    if studio_id <= 0 {
        Err(ShowtimeError::Validation("Studio ID harus > 0".to_string()))
    } else {
        Ok(())
    }
}
...
fn validate_price(price: Decimal) -> Result<()> {
    if price <= Decimal::ZERO {
        Err(ShowtimeError::Validation("Price harus > 0".to_string()))
    } else if price > Decimal::from(1000000) {
        Err(ShowtimeError::Validation("Price terlalu tinggi (max 1,000,000)".to_string()))
    } else {
        Ok(())
    }
}
...
fn validate_start_time(start_time: NaiveDateTime) -> Result<()> {
    ...
    if start_time < now {
        Err(ShowtimeError::Validation("Start time harus di masa depan".to_string()))
    } else {
        Ok(())
    }
}
...
fn entity_to_model(entity: Showtime) -> crate::models::showtime::Showtime {
    crate::models::showtime::Showtime {
        id: entity.id,
        global_movie_id: entity.movie_id,
...
fn entities_to_models(entities: Vec<Showtime>) -> Vec<crate::models::showtime::Showtime> {
    entities.into_iter()
        .map(entity_to_model)
        .collect()
}

fn build_datetime_with_tz(naive: NaiveDateTime) -> Result<chrono::DateTime<chrono::Local>> {
    use chrono::Local;
...
/// Get all showtimes
pub async fn get_all(db: &DatabaseConnection) -> Result<Vec<crate::models::showtime::Showtime>> {
...
/// Get showtime by ID
pub async fn get_by_id(db: &DatabaseConnection, id: i64) -> Result<crate::models::showtime::Showtime> {
    ShowtimesEntity::find_by_id(id)
        .one(db)
        .await
        .map_err(|e| ShowtimeError::Database(e.to_string()))?
        .map(entity_to_model)
        .ok_or_else(|| ShowtimeError::NotFound(format!("Showtime dengan id {} tidak ditemukan", id)))
}

/// Get showtimes by movie ID
pub async fn get_by_movie(db: &DatabaseConnection, movie_id: i64) -> Result<Vec<crate::models::showtime::Showtime>> {
    use crate::entities::showtimes::Column;
...
pub async fn get_by_studio(db: &DatabaseConnection, studio_id: i64) -> Result<Vec<crate::models::showtime::Showtime>> {
    use crate::entities::showtimes::Column;
...
/// Create new showtime
pub async fn create(db: &DatabaseConnection, request: CreateShowtimeRequest) -> Result<crate::models::showtime::Showtime> {
    validate_movie_id(request.global_movie_id)?;
    ...
    let new_showtime = ActiveModel {
        movie_id: Set(Some(request.global_movie_id)),
...
pub async fn update(
    ...
    if let Some(movie_id) = request.global_movie_id {
    ...
    let updated_movie_id = request.global_movie_id.map(Some).unwrap_or(existing.movie_id);
...
pub async fn get_upcoming(db: &DatabaseConnection) -> Result<Vec<crate::models::showtime::Showtime>> {
    use crate::entities::showtimes::Column;
    use chrono::Local;
...
pub async fn get_today(db: &DatabaseConnection) -> Result<Vec<crate::models::showtime::Showtime>> {
    use chrono::Local;
