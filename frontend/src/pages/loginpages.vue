          ...
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form class="space-y-4 md:space-y-6" @submit.prevent="handleLogin">
                  ...
                  <div class="flex items-center justify-between">
                      <div class="flex items-start">
                          <div class="flex items-center h-5">
                            <input id="remember" aria-describedby="remember" type="checkbox" v-model="remember" class="w-4 h-4 border border-gray-300 rounded bg-white focus:ring-3 focus:ring-blue-300">
                          </div>
                          <div class="ml-3 text-sm">
                            <label for="remember" class="text-gray-600">Keep me logged in</label>
                          </div>
                      </div>
                  ...
                  <p class="text-sm font-light text-gray-600 text-center">
                      Don't have an account? <RouterLink to="/register" class="font-medium text-blue-600 hover:underline">Sign up</RouterLink>
                  </p>
              </form>
          </div>
      </div>
...
const handleLogin = async () => {
  ...
  try {
    ...
    // prefer composable error but normalize
    errorMessage.value = err?.message || error.value || 'Login gagal';
  }
};
</script>
